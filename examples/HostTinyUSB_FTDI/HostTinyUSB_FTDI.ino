...
/*
 * HostTinyUSB_FTDI — Giga USB-A host UART bridge for FTDI FT232R (0403:6001)
 * via the TinyUSB host stack (tuh_cdc + upstream FTDI serial driver).
 *
 * Wiring:
 *   Giga --- USB-A ---> FTDI USB (FT232R 0403:6001, interface FF/FF/FF)
 ...
 *   A. ECHO (default): FTDI TX jumpered to RX. Sketch sends binary seq blocks
 *      0..255 and verifies the loopback echo with seq/CRC counters.
 *   B. CROSS: FTDI TX->D0 (RX1), RX->D1 (TX1), GND->GND. Sketch bridges the
...
static const char *ftdiModeName(int m) {
  switch (m) { case FTDI_ECHO: return "ECHO"; case FTDI_CROSS: return "CROSS"; case FTDI_TX_FLOOD: return "TX_FLOOD"; }
  return "?";
}
...
static void printStatus()
{
  ...
  char line[192];
  snprintf(line, sizeof(line), "[TUH-FTDI] mode=%s baud=%d elapsed=%lus tx=%lu (%.1f KB/s) rx=%lu (%.1f KB/s) ok=%lu bad=%lu err=%lu",
...
static void ftdi_cfg_baud_cb(tuh_xfer_t *xfer)
{
  ...
  {
    char line[64];
    snprintf(line, sizeof(line), "[TUH-FTDI] baud set FAILED result=%d", (int) xfer->result);
    GIGATINYUSB_CONSOLE.println(line);
    return;
  }
...
static void ftdi_cfg_format_cb(tuh_xfer_t *xfer)
{
  ...
  {
    char line[64];
    snprintf(line, sizeof(line), "[TUH-FTDI] format set FAILED result=%d", (int) xfer->result);
    GIGATINYUSB_CONSOLE.println(line);
    return;
  }
  ...
  ftdi_ready = true;
  resetCounters();
  GIGATINYUSB_CONSOLE.println("[TUH-FTDI] ready (baud+8N1+DTR+RTS)");
}
...
{
  if (b == ftdi_baud && ftdi_ready) return;
  ftdi_baud = b;
  char line[48];
  snprintf(line, sizeof(line), "[TUH-FTDI] baud=%d", b);
  GIGATINYUSB_CONSOLE.println(line);
  if (tuh_cdc_mounted(0)) ftdi_configure(b);
  resetCounters();
}
...
void setup()
{
  ...
  delay(1000);
  GIGATINYUSB_CONSOLE.println("\n### Giga TinyUSB FTDI bridge ###");
  GIGATINYUSB_CONSOLE.println("Commands: e=ECHO(TX-RX jumper) c=CROSS(FTDI<->Serial1) t=TX_FLOOD 9=9600 1=115200 X=VBUS POR R=re-configure");
  Serial1.begin(115200); // for CROSS mode: FTDI TX->D0 RX->D1 GND->GND
  tuh_giga_init();
}
...
void loop()
{
    ...
    if (c == '\r' || c == '\n')
    {
      if (cmdLen == 1)
      {
        ...
        else if (k == 'X' || k == 'x')
        {
          ...
          GIGATINYUSB_CONSOLE.println("[TUH-FTDI] VBUS power-cycle...");
          digitalWrite(GIGATINYUSB_VBUS_PIN, LOW);
          delay(1000);
          gigaTinyUSB_enableHostPort();
          delay(500);
          GIGATINYUSB_CONSOLE.println("[TUH-FTDI] VBUS restored, waiting for mount...");
        }
        else if (k == 'R' || k == 'r')
        {
          // Re-run the SIO configure chain on demand (recovery without
          // replug): baud -> 8N1 -> DTR+RTS.
          GIGATINYUSB_CONSOLE.println("[TUH-FTDI] re-configure...");
          if (tuh_cdc_mounted(0))
            ftdi_configure(ftdi_baud);
          else
            GIGATINYUSB_CONSOLE.println("[TUH-FTDI] not mounted");
        }
        if (nm != ftdiMode)
        {
          ftdiMode = nm;
          ...
          GIGATINYUSB_CONSOLE.print("[TUH-FTDI] switched to ");
  ...
  {
    char tbuf[96];
    snprintf(tbuf, sizeof(tbuf), "tick conn=%d rx=%lu tx=%lu",
      tuh_cdc_mounted(0) ? 1 : 0, (unsigned long)rx_bytes_total,
  ...
  switch (ftdiMode) {
        ...
        for (uint32_t i = 0; i < n; i++)
        {
          ...
          if (rxbuf[i] != exp)
          {
            ...
            if (rx_errors <= 12)
            {
              char dbg[128];
              int o = 0;
              o += snprintf(dbg + o, sizeof(dbg) - o, "[TUH-FTDI] slip exp=%02x got=", exp);
    ...
    case FTDI_CROSS: {
      ...
      static uint32_t a_seq = 0, a_exp = 0, b_seq = 0, b_exp = 0;
      static uint32_t a_err = 0, b_err = 0;
      ...
      {
        char xbuf[128];
        snprintf(xbuf, sizeof(xbuf), "[TUH-FTDI] cross A(dir USB->D0) err=%lu B(dir D1->USB) err=%lu",
...
{
  (void) daddr;
  GIGATINYUSB_CONSOLE.println("[TUH-FTDI] device mounted");
}
...
void tuh_umount_cb(uint8_t daddr)
{
  (void) daddr;
  ftdi_ready = false;
  GIGATINYUSB_CONSOLE.println("[TUH-FTDI] device unmounted");
}

void tuh_cdc_mount_cb(uint8_t idx)
{
  (void) idx;
  GIGATINYUSB_CONSOLE.println("[TUH-FTDI] cdc mounted");
  ftdi_configure(ftdi_baud);
}
